...
class LiteralNode(ExprNode):
    ...
    def __init__(self, literal: str,
        ...
        if literal in ('true', 'false'):
            ...
            self.value = self._is_float(literal)
    ...
    def _parse_number(self, s: str) -> Union[int, float]:
        ...
        # Убираем суффиксы 'f', 'F', 'd', 'D'
        if s[-1].lower() in ('f', 'd'):
            s = s[:-1]

        # Преобразуем в float, если есть точка или экспонента
...
class AssignNode(ExprNode):
    ...
    def __init__(self, op: AssignOp, var: IdentNode, val: ExprNode,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.op = op
        self.var = var
...
class WhileNode(StmtNode):
    ...
    def __init__(self, cond: Optional[ExprNode], body: Optional[StmtNode],
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.cond = cond
        self.body = body if body else EMPTY_STMT
...
class SwitchELNode(StmtNode):
    """Класс для представления в AST-дереве switch
    """
    pass
class CaseNode(SwitchELNode):
    """Класс для представления в AST-дереве case
    """
    def __init__(self, init: Optional[ExprNode], body: Optional[StmtNode],
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.init = init
        self.body = body if body else EMPTY_STMT
...
class DefaultNode(SwitchELNode):
    """Класс для представления в AST-дереве default
    """
...
class SwitchNode(StmtNode):
    """Класс для представления в AST-дереве switch
    """
    def __init__(self, init: Optional[StmtNode], body: Optional[CaseNode],
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        super().__init__(row=row, col=col, **props)
        self.init = init
    ...
    @property
    def childs(self) -> Tuple[AstNode, ...]:
        return (self.init,) + tuple(self.body)

    def semantic_check(self, scope: IdentScope) -> None:
        print(f"Checking semantic for SwitchNode at row {self.row}, col {self.col}")
        self.init.semantic_check(scope)
        self.body.semantic_check(scope)
        self.node_type = TypeDesc.VOID
